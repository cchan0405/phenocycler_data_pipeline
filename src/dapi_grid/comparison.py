...
from .clustering import _elbow, _plot_selection
from .config import ComparisonConfig
from .render import render_cluster_overlay
...
def _load_sample(sample, group):
    table_path = sample.results_dir / "grid_clusters.csv"
    summary_path = sample.results_dir / "run_summary.json"
    if not table_path.exists() or not summary_path.exists():
        raise FileNotFoundError(
            f"{sample.name}: expected grid_clusters.csv and run_summary.json in "
...
def _validate_compatibility(records):
    ...
    reference_features = model_columns(records[0][1])
    for name, frame, summary in records[1:]:
        ...
        if model_columns(frame) != reference_features:
            raise ValueError(f"{name} uses a different window-feature schema")
    return reference, reference_features


def _spatial_candidates(frame, window_size, stride):
    spacing = max(1, math.ceil(window_size / stride))
    positions = np.flatnonzero(
        frame.grid_row.mod(spacing).eq(0).to_numpy()
        & frame.grid_col.mod(spacing).eq(0).to_numpy()
    )
    return positions if len(positions) >= 3 else np.arange(len(frame))


def _balanced_training(records, signature, maximum, seed):
    rng = np.random.default_rng(seed)
    candidates = [
        _spatial_candidates(frame, signature["window_size_px"], signature["stride_px"])
        for _, frame, _ in records
    ]
    per_image = min(maximum, min(map(len, candidates)))
    if per_image < 3:
        raise ValueError("Every image needs at least three spatially separated windows")
    selections = [
...
def _fit_joint(records, features, signature, cfg, output):
    per_image, selections = _balanced_training(
        records, signature, cfg.max_training_windows_per_image, cfg.random_seed
    )
    training = pd.concat(
    ...
    selected_k = elbow_k if close else int(best.k)
    ...
    _plot_selection(scores, selected_k, output / "joint_cluster_selection.png")
    ...
    return labelled, {
        ...
        "stable": stability >= .80, "training_windows_per_image": per_image,
        "total_balanced_training_windows": len(training),
...
def _summary_table(labelled, control_name):
    spatial = {frame["sample"].iloc[0]: _spatial_metrics(frame) for frame in labelled}
    proportions = []
    for frame in labelled:
        name = frame["sample"].iloc[0]
        counts = frame.cluster.value_counts(normalize=True)
        for cluster in sorted(pd.concat(labelled).cluster.unique()):
            proportions.append({
                "sample": name, "group": frame["group"].iloc[0], "cluster": cluster,
                "proportion": float(counts.get(cluster, 0)),
                "median_nuclei_per_window": float(frame.n_nuclei.median()),
                **spatial[name],
            })
...
def run_comparison(cfg: ComparisonConfig):
    ...
    combined.to_csv(cfg.output_dir / "joint_grid_clusters.csv", index=False)
    results = _summary_table(labelled, cfg.control.name)
    ...
    for (name, _, summary), frame in zip(records, labelled):
        ...
        render_cluster_overlay(
            preview, frame, image_shape=tuple(summary["image_shape_level0"]),
