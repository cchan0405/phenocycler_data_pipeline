...
def render_cluster_overlay(preview, grid_df, *, image_shape, stride, alpha, path):
    ...
    bar.set_label("Nuclear-morphology environment")
    ax.set_title("Whole-tissue nuclear-morphology clusters")
    ax.set_axis_off()
    fig.tight_layout()
...
def render_segmentation_ometiff(
    ...
    """Write a tiled pyramid with true StarDist outlines on the original DAPI."""
    ...
    def polygons_for_box(y0, x0, y1, x1):
        ...
        for chunk_row in range(row0, row1 + 1):
            for chunk_col in range(col0, col1 + 1):
                polygons = load_polygons(chunk_row * n_chunk_cols + chunk_col)
    ...
    def tiles_for_level(level):
        ...
        for y in range(0, height, tile_size):
            for x in range(0, width, tile_size):
                ...
                for polygon in polygons_for_box(*box0):
                    py, px = polygon
                    points = [((float(xx) / scale_x) - x, (float(yy) / scale_y) - y)
                              for yy, xx in zip(py, px)]
                    if points:
                        draw.line(points + [points[0]], fill=(0, 255, 255),
