...
def keep_owned_nuclei(df: pd.DataFrame, chunk) -> pd.DataFrame:
    """Keep every StarDist detection owned by the non-overlapping chunk core.

    Ownership filtering is not biological quality control. It prevents nuclei
    detected in neighbouring halo regions from being counted more than once.
    No area, solidity, shape, or DAPI-intensity thresholds are applied here.
    """
    if df.empty:
        return df
    out = df.copy()
