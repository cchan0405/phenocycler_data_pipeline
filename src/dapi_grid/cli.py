...
def parser() -> argparse.ArgumentParser:
    ...
    run.add_argument(
        "--rerender-segmentation", action="store_true",
        help="Rebuild the coloured segmentation OME-TIFF without rerunning StarDist",
    )
...
def main() -> None:
    ...
    if args.command == "run":
        run_pipeline(
            load_config(args.config), force=args.force,
            rerender_segmentation=args.rerender_segmentation,
        )
    else:
